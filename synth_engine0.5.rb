...
def pad(note, duration)
  ...
  envelope(samples, attack: 0.3, decay: duration/2)
    .map { |s| s * 0.25 }
end

def bass(note, duration)
  samples = wave(note_freq(note), duration, :sine)
  envelope(samples, attack: 0.05, decay: duration/2)
    .map { |s| s * 0.35 }
end
...
def piano(note, duration)
  ...
  blended = sine.zip(tri).map { |a,b| (a*0.8 + b*0.2) }

  envelope(blended, attack: 0.01, decay: duration * 0.8)
    .map { |s| s * 0.35 }
end
...
def soft_kick
  ...
  envelope(shaped, attack: 0.01, decay: 0.3)
    .map { |s| s * 0.3 }
end
...
# 🌸 Simple calm piano melody (1 note per bar)
piano_melody = ["E5", "C5", "A4", "G4"]
...
LOOPS.times do
  chord_progression.each_with_index do |chord, i|
    ...
    # Soft kick (once per bar)
    layers << soft_kick

    # 🎹 Gentle piano on top
    layers << piano(piano_melody[i], BAR)
...
filename = "nightwish_v1#{Time.now.to_i}.wav"
